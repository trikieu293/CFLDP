...
def cfldp(n_customer, alpha, beta, lamda, theta):
    ...
    # Check result in exact model
    model_exact = gp.Model()
    x_index = [(j, r) for j in S for r in R.keys()]

    # Decision varibales
    x_exact = model_exact.addVars(x_index, vtype=GRB.INTEGER, name='x_exact')
    u1 = model_exact.addVars([i for i in N], vtype=GRB.CONTINUOUS, name='u1')
    u2 = model_exact.addVars([i for i in N], vtype=GRB.CONTINUOUS, name='u2')
    u3 = model_exact.addVars([i for i in N], vtype=GRB.CONTINUOUS, name='u3')
    u4 = model_exact.addVars([i for i in N], vtype=GRB.CONTINUOUS, name='u4')

    for i in N:
        u1[i].LB = get_u_c(i)
        u1[i].UB = get_u_c(i) + get_max_u_s(i)
    # Objective Function
    model_exact.setObjective(sum(w[i] * (1 - u4[i]) * u3[i] for i in N), GRB.MAXIMIZE)

    # Constraints
    for j in S:
        model_exact.addConstr(sum(x_exact[j, r] for r in R.keys()) <= 1, name="Constraints 1")

    model_exact.addConstr(sum(sum(get_cost(r) * x_exact[j, r] for r in R.keys()) for j in S) <= 5, name="Constraints 2")

    for i in N:
        model_exact.addConstr(u1[i] == get_u_c(i) + sum(x_exact[j, r] * get_utility(i, j, r) for j in S for r in R.keys()), name="ConstrU1")
        model_exact.addConstr(u1[i] * u2[i] == 1.0, name="ConstrU2")
        model_exact.addConstr(u3[i] == 1.0 - (get_u_c(i) * u2[i]), name="ConstrU3")
        e_lambda = math.exp(-LAMBDA)
        model_exact.addGenConstrExpA(u1[i], u4[i], e_lambda, name="ConstrU4")

    for index, row in x_result.iterrows():
        model_exact.addConstr(x_exact[row["j"], row["r"]] == 1, name="Constraints test")

    model_exact.Params.TimeLimit = 60 * 60
    # model.params.FuncPieces = 1000
    model_exact.update()
    model_exact.optimize()


    x_exact_result = pd.DataFrame(x_exact.keys(), columns=["j", "r"])
    x_exact_result["value"] = model_exact.getAttr("X", x_exact).values()
    x_exact_result["cost"] = [get_cost(r) for r in x_exact_result["r"]]
    x_exact_result["attractiveness"] = [get_attractiveness(r) for r in x_exact_result["r"]]
    x_exact_result.drop(x_exact_result[x_exact_result.value < 0.9].index, inplace=True)

    # return [x_result, y_result, a_dict, b_dict, c_dict, l_dict]
    return [model_exact.getObjective().getValue(), x_exact_result, x_result]
def exact(n_customer, beta, lamda, theta):
    ...
    return [model.getObjective().getValue(), x_result]
...
if __name__ == "__main__":
    ...
    for i in range(50, 51):
        ...
        result_set.append((result_exact[0] - result_approximation[0]) / result_exact[0])
        print(result_approximation[2])
        print(result_approximation[1])
        print(result_exact[1])
    print(result_set)
    print("End")